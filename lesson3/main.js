...
let i = 0;
while (i < 20) {
    document.write(`<div><h1>How are you?)</h1></div>`)
    i++;
}
...
let a = 0;
while (a < 20) {
    document.write(`<div><h1>Just Text ${[a]}  </h1></div>`)
    a++;
}
...
for (let j = 0; j < products.length; j++) {
    ...
    document.write(`<h3 class="product-title">${product.title} ${product.price} -  UAN</h3> 
...
for (let j = 0; j < users.length; j++) {
    if (users[j].status === true )
        document.write(`<h4>${users[j].name}</h4>`)
